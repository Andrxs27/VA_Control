...
// ── 6. CARGAR PERFIL DE USUARIO LOGUEADO ─────────────────────────────────────
async function cargarPerfilUsuario() {
    try {
        // NOTA: Ajusta '/usuarios/perfil' si el endpoint de tu backend es diferente (ej: '/auth/me')
        ...
        // 2. Generar e inyectar Iniciales para el Avatar (Ej: "Admin User" -> "AU")
        ...
        if (avatarEl && usuario.nombre) {
            const iniciales = usuario.nombre
                ...
                .join('')
                .toUpperCase()
                .slice(0, 2); // Tomamos máximo 2 letras
            avatarEl.innerText = iniciales || '??';
        }

        // 3. Actualizar Rol traduciendo dinámicamente con tu función _t
        ...
        if (rolEl) {
            ...
            rolEl.innerText = mapRoles[usuario.rol] || usuario.rol;
            rolEl.setAttribute('data-i18n', `rol_${usuario.rol}`);
        }

    } catch (error) {
        console.error("Error al cargar los datos del menú lateral:", error);
        // Opcional: Mostrar valores por defecto de error si falla
        const nombreEl = document.getElementById('perfil-nombre');
        if (nombreEl) nombreEl.innerText = _t('Sesión Expirada', 'Session Expired');
    }
}

// Ejecutar la carga del perfil al iniciar el script
cargarPerfilUsuario();